...
namespace AsmdefHelper.DependencyGraph.Editor.DependencyNode
{
    public class DomainGroup
    {
        ...
        public IEnumerable<string> GetSoloDomains() =>
            dict.Where(x => x.Value.Count == 1).Select(x => x.Key);

        public IEnumerable<string> GetTopDomainsWithSomeSubDomains() =>
            dict.Keys.Except(GetSoloDomains());
